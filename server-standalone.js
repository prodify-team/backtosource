// Standalone server for Back to Source Chatbot
...
// Chat endpoint with inline responses
app.post('/api/chat', (req, res) => {
  try {
    ...
    const response = getResponse(message, finalUserRole, finalUserName);
    ...
    res.json({
      response: `Sorry ${req.body?.userName || 'Ji'}! Technical issue hai. Please try again.`,
      timestamp: new Date().toISOString(),
      error: 'fallback'
    });
  }
...
app.listen(PORT, () => {
  console.log(`🚀 Back to Source Chatbot running on port ${PORT}`);
});
